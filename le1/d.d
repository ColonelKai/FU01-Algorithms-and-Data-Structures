...
The idea is as follows:
    ...
    On xth step, the three tasks;
        Calculate Delta(X-Min) : 
            Get the xth min value (eg. if the list is sorted in increasing order as (a1, a2, a3, a4 ... an), choose ax), and the absolute max order.
            Then get the differnece between those two values, hence getting Delta(X-Min)
            
        Calculate Delta(X-Max) : 
            Get the xth max value (eg. if the list is sorted in decreasing order as (a1, a2, a3, a4 ... an), choose ax), and the absolute min order.
            Then get the difference between those two values, hence getting Delta (X-Max)

        THEN;
            If neither Delta(X-min) or Delta(X-max) are positive, increment x.
            If either Delta(X-min) or Delta(X-max) are positive, choose it.
            If both Delta(X-min) and Delta(X-max) are positive, choose the larger one.
*/
...
int findMinPrice(int[] arr, int min) {
    ...
    foreach(ind;0 .. (arr.length)-1) {
        if(arr[ind]<=arr[resInd] && arr[ind] > mincomplexity) {
            resInd = ind;
        }
    }
    return resInd;
